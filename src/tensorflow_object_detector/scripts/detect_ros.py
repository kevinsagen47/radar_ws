...
with detection_graph.as_default():
    od_graph_def = tf.compat.v1.GraphDef()#tf.compat.v1.GraphDef()   # -> instead of tf.GraphDef()
    with tf.compat.v2.io.gfile.GFile(PATH_TO_CKPT, 'rb') as fid:#tf.compat.v2.io.gfile.GFile()   # -> instead of tf.gfile.GFile()
...
config = tf.compat.v1.ConfigProto()#tf.ConfigProto by tf.compat.v1.ConfigProto
...
class Detector:
    ...
    def image_cb(self, data):
        ...
        #self.sess.run([boxes, scores, classes, num_detections],
        #(boxes, scores, classes, num_detections) =  tf.compat.v1.Session(graph=detection_graph,config=config).run([boxes, scores, classes, num_detections],
